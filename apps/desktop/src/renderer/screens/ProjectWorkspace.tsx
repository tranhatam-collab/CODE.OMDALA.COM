      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        >
          ...
          {files.map((f) => (
            <div
              key={f.path}
              onClick={() => (f.isDirectory ? null : handleFileClick(f.path))}
              style={{
                padding: '4px 8px',
                borderRadius: '4px',
                cursor: f.isDirectory ? 'default' : 'pointer',
                fontSize: '13px',
                background: selectedFile === f.path ? '#3b82f620' : 'transparent',
                color: f.isDirectory ? '#8b8ba7' : '#e2e8f0',
              }}
            >
        ...
        <main style={{ flex: 1, overflow: 'auto', padding: '16px' }}>
          {activeTab === 'files' &&
            (selectedFile ? (
              <CodeEditor value={fileContent} readOnly />
            ) : (
              <p style={{ color: '#8b8ba7' }}>Select a file to view</p>
            ))}
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/v1/ai/chat`, {
        ...
        body: JSON.stringify({
          task: 'chat',
          context: { projectId: project.id, userMessage: input },
        }),
      });
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: 'Error: Cannot connect to API' },
      ]);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flex: 1, overflow: 'auto', marginBottom: '12px' }}>
        {messages.length === 0 && (
          <p style={{ color: '#8b8ba7', textAlign: 'center', padding: '32px' }}>
            {t('empty.sessions', locale)}
          </p>
        )}
        {messages.map((m, i) => (
          <div
            ...
            style={{
              ...
              marginBottom: '8px',
              borderRadius: '8px',
              background: m.role === 'user' ? '#3b82f620' : '#2d2d44',
              maxWidth: '80%',
            }}
          >
