...
export default function DiagnosticsScreen({ locale, onBack }: Props) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.electronAPI) {
      Promise.all([window.electronAPI.getVersion(), window.electronAPI.getPlatform()]).then(([version, platform]) => setInfo({ version, platform }));
    }
  }, []);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <main style={{ flex: 1, overflow: 'auto', padding: '32px', maxWidth: '480px', margin: '0 auto', width: '100%' }}>
        <pre style={{ fontSize: '13px', color: '#e2e8f0', background: '#1e1e3a', padding: '16px', borderRadius: '8px' }}>{JSON.stringify(info, null, 2)}</pre>
      </main>
    </div>
  );
}
