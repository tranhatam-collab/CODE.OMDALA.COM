...
export default async function HomeScreen({
  ...
  const handleOpenFolder = async () => {
    if (typeof window !== 'undefined' && window.electronAPI) {
      ...
      if (path) {
        const _isRepo = await window.electronAPI.isGitRepo(path);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      >
        ...
        <nav style={{ display: 'flex', gap: '16px' }}>
          <button onClick={() => onNavigate('providers')} style={navBtn}>
            {t('nav.providers', locale)}
          </button>
          <button onClick={() => onNavigate('settings')} style={navBtn}>
            {t('nav.settings', locale) || 'Settings'}
          </button>
        </nav>
      </header>
      <main style={{ flex: 1, overflow: 'auto', padding: '32px' }}>
        <section style={{ marginBottom: '32px' }}>
          <h2 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '16px' }}>
            {t('home.recent_projects', locale) || 'Recent Projects'}
          </h2>
          {projects.length === 0 ? (
            <div
              style={{
            ...
            >
              <p style={{ color: '#8b8ba7', marginBottom: '16px' }}>
                {t('empty.projects', locale)}
              </p>
              <button onClick={handleOpenFolder} style={primaryBtn}>
                {t('cta.open_project', locale) || 'Open Project'}
              </button>
            </div>
          ) : (
            <div
        ...
        <section>
          <h2 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '16px' }}>
            {t('home.connected_providers', locale) || 'Connected Providers'}
          </h2>
          {providers.length === 0 ? (
              ...
              {providers.map((p, i) => (
                <div key={i} style={providerCard}>
                  ...
                  <span
                    style={{
                      ...badge,
                      background: p.status === 'valid' ? '#22c55e20' : '#ef444420',
                      color: p.status === 'valid' ? '#22c55e' : '#ef4444',
                    }}
                  >
                    {p.status}
                  </span>
