...
app.get('/health', async (c) => {
  try {
    ...
    return c.json({ status: 'degraded', db: 'disconnected' }, 500);
  }
});
...
ai.post('/plan', async (c) => {
  ...
  try {
    ...
    // Log action to D1
    await c.env.DB.prepare('INSERT INTO audit_logs (id, action, object_id) VALUES (?, ?, ?)').bind(crypto.randomUUID(), 'ai_plan', req.context.projectId).run();
