...
export interface FileEntry { name: string; path: string; type: 'file' | 'dir'; size?: number; }
export interface BranchInfo { name: string; sha: string; protected: boolean; }
export interface CommitInfo { sha: string; message: string; author: string; date: string; }
export interface PullRequestInfo { number: number; title: string; state: string; url: string; }
...
export class GitHubMcpServer {
  ...
  async listBranches(): Promise<BranchInfo[]> {
    const res = await this.fetch(`/repos/${this.config.owner}/${this.config.repo}/branches?per_page=100`);
  ...
  async readFile(path: string, branch?: string): Promise<string> {
    ...
    const res = await this.fetch(`/repos/${this.config.owner}/${this.config.repo}/contents/${path}?ref=${ref}`);
  ...
  async listFiles(dirPath: string, branch?: string): Promise<FileEntry[]> {
    const ref = branch || 'main';
    const res = await this.fetch(`/repos/${this.config.owner}/${this.config.repo}/contents/${dirPath}?ref=${ref}`);
    const items = Array.isArray(res) ? res : [res];
    return items.map((item: any) => ({ name: item.name, path: item.path, type: item.type, size: item.size }));
  }

  async createBranch(branchName: string, baseSha?: string): Promise<{ name: string; sha: string }> {
    const repo = await this.readRepo();
    const baseRef = await this.fetch(`/repos/${this.config.owner}/${this.config.repo}/git/ref/heads/${repo.defaultBranch}`);
  ...
  async createPr(title: string, body: string, head: string, base?: string): Promise<PullRequestInfo> {
  ...
  async listCommits(sha?: string, perPage: number = 30): Promise<CommitInfo[]> {
    ...
    const res = await this.fetch(`/repos/${this.config.owner}/${this.config.repo}/commits?${params}`);
  ...
  async getCommitDiff(sha: string): Promise<string> {
    const res = await fetch(`${this.baseUrl}/repos/${this.config.owner}/${this.config.repo}/commits/${sha}`, {
      headers: { Authorization: `Bearer ${this.config.token}`, Accept: 'application/vnd.github.v3.diff' },
    });
    return res.text();
  }
