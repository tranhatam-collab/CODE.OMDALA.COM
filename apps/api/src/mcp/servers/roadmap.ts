export interface RoadmapItem { id: string; title: string; status: 'todo' | 'in_progress' | 'done'; phase: number; }
...
export class RoadmapMcpServer {
  ...
  async updateRoadmap(itemId: string, status: 'todo' | 'in_progress' | 'done'): Promise<{ success: boolean; item: RoadmapItem }> {
    // Requires approval
  ...
  async generateRoadmap(issuesOrPrs: string[]): Promise<RoadmapItem[]> {
    return issuesOrPrs.map((i, index) => ({ id: `auto-${index}`, title: i, status: 'todo', phase: 1 }));
  }
