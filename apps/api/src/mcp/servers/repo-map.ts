...
export class RepoMapMcpServer {
    ...
    for (const filePath of targetFiles) {
      try {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          const patterns = [
            {
              regex: new RegExp(`(?:export\\s+)?(?:async\\s+)?function\\s+${symbolName}`),
              type: 'function' as const,
            },
            { regex: new RegExp(`(?:export\\s+)?class\\s+${symbolName}`), type: 'class' as const },
            {
              regex: new RegExp(`(?:export\\s+)?interface\\s+${symbolName}`),
              type: 'interface' as const,
            },
            {
              regex: new RegExp(`(?:export\\s+)?(?:const|let|var)\\s+${symbolName}`),
  ...
  async getDependencies(filePath: string): Promise<string[]> {
    ...
    try {
      ...
      return imports
        .map((i) => i.replace(/(?:import|require)\s*[(]?\s*['"]/, '').replace(/['"]/, ''))
        .filter(Boolean);
    } catch {
      return [];
    }
