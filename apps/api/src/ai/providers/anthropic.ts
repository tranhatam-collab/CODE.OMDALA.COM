...
export class AnthropicAdapter implements ProviderAdapter {
  ...
  async getModels() {
    return ['claude-opus-4-20250514', 'claude-sonnet-4-20250514', 'claude-haiku'];
  }
  async getHealth() {
    return { status: 'healthy' as const };
  }
  getCost(inputTokens: number, outputTokens: number, model: string) {
    return estimateCost(this.name, model, inputTokens, outputTokens);
  }
}
